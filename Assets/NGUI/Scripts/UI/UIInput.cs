...
public class UIInput : MonoBehaviour
{
	...
	/// <summary>
	/// Helper function that retrieves the str of the character under the mouse.
	/// </summary>
	...
	protected char Validate (string text, int pos, char ch)
	{
		...
		else if (validation == Validation.QxUserName)//新加代码 0-9 a-z A-Z 
		{
			if (ch >= 'A' && ch <= 'Z') return ch;
			if (ch >= 'a' && ch <= 'z') return ch;
			if (ch >= '0' && ch <= '9') return ch;
//			if (ch>=0x4e00 && ch<=0x9fa5) return ch;//这个主要是汉字的范围
		...
		else if (validation == Validation.QxPassWord)
		{
			if (ch >= 'A' && ch <= 'Z') return ch;
			if (ch >= 'a' && ch <= 'z') return ch;
			if (ch >= '0' && ch <= '9') return ch;
